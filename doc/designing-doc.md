...
![学生用户的用例和子用例](./img/use-case-0.png)
图2-2 学生用户的用例和子用例

![教师用户的部分用例和子用例](./img/use-case-1.png)
图2-3 教师用户的部分用例和子用例

![管理员用户的部分用例和子用例](./img/use-case-2.png)
图2-4 管理员用户的部分用例和子用例
...
![注册用例的活动图](./img/activity-diag-1.png)
图2-1 登录用例的活动图
...
![TutorialApp客户端的架构](./img/artecture.png)
图2-xx TutorialApp客户端的架构
...
图 2-xx TutorialApp中注册用例的类的设计

![TutorialApp项目关键分析类](./img/use-case-impl-1.png)
图 2-xx TutorialApp项目关键分析类
...
![图片](https://images-cdn.shimo.im/8SwIq1d4weQaZEil/image.png)

图 2-xx  TutorialApp用户管理子系统类的设计

### 2.5.2 TutorialApp订单管理子系统

### 2.5.3 TutorialApp微博管理子系统

### 2.5.4 分析类到设计元素的映射

至此，对TutorialApp客户端的设计基本完成了，先后分别完成了分析类、设计类、子系统的设计工作。为了更加清晰地从分析类到设计元素的映射，列出如下设计表。
表2-xx 分析类到设计元素的映射表
